...
mex BootstrapMeanCublasGPU.cpp -lcudart -lcurand -lcublas -lStatistics -IC:/Program' Files'/NVIDIA' GPU Computing Toolkit'/CUDA/v5.0/include -LC:/Program' Files'/NVIDIA' GPU Computing Toolkit'/CUDA/v5.0/lib/x64 -LC:/users/wande/Documents/Visual' Studio 2010'/Projects/Statistics/x64/Release/ -IC:/users/wande/Documents/Visual' Studio 2010'/Projects/Statistics/Statistics
...
%newdata = zeros(Nbootstraps,Nsamples);
%start = clock;   
%for b = 1:Nbootstraps
%    indices = randi(Nsamples,Nsamples,1);
%    new = data(indices);
%    newdata(b,:) = new;    
%end
%indices = randi(Nsamples,Nsamples,Nbootstraps);
%newdata = data(indices)';
%means2 = newdata * 1/Nsamples*ones(Nsamples,1);
%CPU_time2 = etime(clock,start)

[means_GPU,GPU_time] = BootstrapMeanCublasGPU(data,Nbootstraps); 
